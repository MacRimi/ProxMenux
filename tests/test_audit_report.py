...
class StoreTests(unittest.TestCase):
    ...
    def test_acceptance_is_not_resolution_and_class_changes_are_visible(self):
        ...
        worse = engine.compare_runs(before, changed)
        self.assertEqual(worse['new'], [])
        self.assertEqual(worse['resolved'], [])
        self.assertEqual(len(worse['worse']), 1)
        self.assertEqual(worse['worse'][0]['classification'], 'critical')
        self.assertEqual(worse['worse'][0]['previous_classification'], 'warning')
        self.assertEqual(worse['worse'][0]['previous_affected'], 1)
        self.assertEqual(worse['worse'][0]['affected_count'], 1)
        self.assertEqual(worse['better'], [])
        better = engine.compare_runs(changed, before)
        self.assertEqual(better['new'], [])
        self.assertEqual(better['resolved'], [])
        self.assertEqual(len(better['better']), 1)
        self.assertEqual(better['better'][0]['classification'], 'warning')
        self.assertEqual(better['better'][0]['previous_classification'], 'critical')
        self.assertEqual(better['better'][0]['previous_affected'], 1)
        self.assertEqual(better['better'][0]['affected_count'], 1)
        self.assertEqual(better['worse'], [])

    def test_new_failure_and_verified_resolution_stay_distinct(self):
        conformant = self.recorded(self.finding(state='pass'))
        warning = self.recorded(self.finding())
        new = engine.compare_runs(conformant, warning)
        self.assertEqual(len(new['new']), 1)
        self.assertEqual(new['new'][0]['classification'], 'warning')
        self.assertEqual(new['resolved'], [])
        self.assertEqual(new['unverified'], [])
        resolved = engine.compare_runs(warning, conformant)
        self.assertEqual(len(resolved['resolved']), 1)
        self.assertEqual(resolved['resolved'][0]['classification'], 'conformant')
        self.assertEqual(resolved['new'], [])
        self.assertEqual(resolved['unverified'], [])
